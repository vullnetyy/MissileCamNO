...
```
 VS Code + dotnet CLI ─build─► MissileFollowCam.dll
      │                              │
      │ (local test)                 │ (release)
      ▼                              ▼
...
```powershell
mkdir C:\dev\MissileFollowCam
cd C:\dev\MissileFollowCam
dotnet new classlib -n MissileFollowCam -o .
git init
code .
```
...
Replace `MissileFollowCam.csproj` entirely with this:
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <PropertyGroup>
    ...
    <AssemblyName>MissileFollowCam</AssemblyName>
...
| Missile launched/expired events | `Aircraft.onRegisterMissile` / `Aircraft.onDeregisterMissile` (both `Action<Missile>`) | **[CONFIRMED]** |
| Missile owner | `Missile.owner` (`Unit`) and `Missile.ownerID` (`PersistentID`) | **[CONFIRMED]** |
| Missile alive flag | `Missile.disabled` (bool; true once detonated) | **[CONFIRMED]** |
...
| Return‑to‑cockpit key | The game's native "Switch View" (default **L**) — verify in game Settings → Controls | **[VERIFY]** |
...
```
Expect `bin\Debug\MissileFollowCam.dll`. Fix any missing‑reference errors by re‑checking `<GameDir>`.
...
1. Run the script → BepInEx console prints `MissileFollowCam v1.0.0 loaded.`
...
# Create an EMPTY public repo 'MissileFollowCam' on GitHub, then:
git remote add origin https://github.com/<YOUR_GITHUB_USERNAME>/MissileFollowCam.git
git branch -M main
    ...
    steps:
      ...
      - name: Package
        ...
        run: |
          ...
          $out = "MissileFollowCam-$ver"
          New-Item -ItemType Directory -Force -Path dist/$out | Out-Null
          Copy-Item "bin/Release/MissileFollowCam.dll" "dist/$out/"
...
```
The workflow builds, zips `MissileFollowCam-1.0.0.zip`, writes the `sha256:` file, and creates the
...
Fork **https://github.com/KopterBuzz/NOMNOM**. In `modManifests/`, add `MissileFollowCam.json`
...
```json
{
  ...
  "githubRepo": "MissileFollowCam",
  "autoUpdate": "True",
  "artifacts": [
    {
      "type": "plugin",
      "fileName": "MissileFollowCam-1.0.0.zip",
      "downloadUrl": "https://github.com/<YOUR_GITHUB_USERNAME>/MissileFollowCam/releases/download/v1.0.0/MissileFollowCam-1.0.0.zip",
      "gameVersion": "0.32",
      "modVersion": "1.0.0",
